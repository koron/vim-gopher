...
call server#start('gopher', s:dir . '/proc/gopher.vim')
sleep 500m
call remote_foreground('gopher')
...
function! s:ario_set(x, y, p)
  call remote_expr('gopher', printf('sprite#update(%d,%d,%d)', a:x, a:y, a:p))
endfunction
...
while 1
  ...
  " select pattern
  if ario.v > 0
    let ario.p = (ario.q / 3) + 0
    let ario.q = (ario.q + 1) % 9
  elseif ario.v < 0
    let ario.p = (ario.q / 3) + 3
    let ario.q = (ario.q + 1) % 9
  elseif ario.jump.jumping
    if ario.p < 3
      let ario.p = (ario.q / 3) + 0
      let ario.q = (ario.q + 1) % 9
    else
